...
mod_litigii_noi_server <- function(input, output, session, vals){
  ...
  observeEvent(input$banca_litigiu_nou,{req(input$banca_litigiu_nou != "none")
    ...
    output$show_beneficiar <- renderUI({req(input$banca_litigiu_nou != "none", input$show_more==1)
      tagList(selectInput(session$ns("beneficiar_litigiu_nou"),label = "Select beneficiar:", width = "400px",
              choices =  c("none",unique(vals_litigii_noi$bi_contracte_banca_selectata$NumeBeneficiar))),
              actionButton(ns("show_more_beneficiari"),label = "Incarca toti beneficiarii. Dureaza putin."), br(), br())
              })
    
   
    output$show_contract   <- renderUI({req(input$banca_litigiu_nou != "none",input$show_more == 1, input$beneficiar_litigiu_nou)
      #req(input$beneficiar_litigiu_nou != "none")
      selectInput(session$ns("contract_litigiu_nou"),label = "Select contractul de garantare:", width = "400px", 
    ...
    output$show_cui_litigiu_nou <- renderUI({req(input$banca_litigiu_nou != "none", input$show_more == 1,
      ...
      selectInput(session$ns("cui_beneficiar_litigiu_nou"),label = "CUI-ul beneficiarului:",width = "400px",
                                choices = c(vals_litigii_noi$bi_contracte_banca_selectata %>% 
                                              dplyr::filter(NumeBeneficiar == input$beneficiar_litigiu_nou) %>%
  ...
  observeEvent(input$save_new_litigiu_manual,{
    shinyWidgets::ask_confirmation(inputId = session$ns("confirm_new_litigiu_manual"),
      ...
      observeEvent(input$confirm_new_litigiu_manual,{
        ...
        vals_litigii_noi$litigiu_nou_manual <- data.frame(Nr_crt = as.integer(max(vals$litigii_curente$Nr_crt)+1),
        ...
        vals$litigii_curente <- dplyr::bind_rows(vals_litigii_noi$litigiu_nou_manual,vals$litigii_curente)
        removeModal(session=session)
      }
